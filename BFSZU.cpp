/*    由22届深大羽协会长Tony Huang(非计算机专业)编写的通用快速抽签分组程序，可应用于日常单项赛或团体赛的抽签分组。
   ...
   《羽毛球竞赛规则》(2021)中“上、下半区”的相关规定进行排布。
      感谢ChatGPT 3.5为代码加上的注释。如需使用或转发请注明来源于深大羽协或Tony Huang
*/
...
bool cmp(const vector<int> &a, const vector<int> &b) // 定义名为 cmp 的 bool 类型函数，用于比较两个 vector 容器的大小
{
    return a.size() < b.size(); // 返回 a 容器大小是否小于 b 容器大小的布尔值，用于排序等操作
}

bool find(int num, vector<int> vec) // 定义名为 find 的 bool 类型函数，用于在 vector 容器中查找指定的元素
{
...
int countNonEmpty(const vector<string> &vec) // 定义名为 countNonEmpty 的整型函数，用于计算 vector 容器中非空字符串的个数
{
...
vector<vector<int>> Draw::divideArray(vector<int> &nums, int m) // 定义名为 divideArray 的函数，返回值为一个二维 vector 容器，用于将一个一维 vector 容器 nums 划分为 m 个子数组
{
...
vector<vector<int>> Draw::deleteSameElements(vector<vector<int>> temp, vector<vector<int>> byeRoundNumber) // 定义名为 deleteSameElements 的函数，返回值为一个二维 vector 容器，用于删除 temp 和 byeRoundNumber 中相同的元素
{
...
void Draw::generation(int option)
{
    ...
    distributeElements(seededArray, tempNameArray);        // 将种子随机数插入分组结果中
